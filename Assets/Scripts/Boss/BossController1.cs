...
public class BossController1 : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        else
        {
            ...
            if (animator == null)
            {
                ...
                if (animator == null)
                {
                    Debug.LogError("No Animator found! Please assign the Animator Override in the inspector.");
                }
            }
        }
        ...
        if (splineSpawner != null)
        {
            ...
            Debug.Log("Successfully connected to SplineBallSpawner for damage reports!");
        }
        else
        {
            Debug.LogWarning("No SplineBallSpawner assigned! Boss won't receive damage from spline matches.");
        }
    ...
    private void Update()
    {
        ...
        if (!isPerformingAttack)
        {
            ...
            if (ShouldPerformJumpAttack(distanceToPlayer))
            {
    ...
    {
        bool healthBelowHalf = currentHealth <= maxHealth / 2;
    ...
    private IEnumerator PerformDashAttack()
    {
        ...
        if (dashChargeEffect) dashChargeEffect.Play();
        ...
        while (windupTimer < dashWindupTime)
        {
            ...
            if (currentDashIndicator)
            {
                float t = windupTimer / dashWindupTime;
                UpdateIndicatorColor(currentDashIndicator, t);
            }
    ...
    private IEnumerator PerformJumpAttack()
    {
        ...
        if (jumpChargeEffect) jumpChargeEffect.Play();
        ...
        while (windupTimer < jumpWindupTime)
        {
            ...
            if (currentJumpIndicator)
            {
                float t = windupTimer / jumpWindupTime;
                UpdateIndicatorColor(currentJumpIndicator, t);
                
                float scale = 1f + Mathf.Sin(windupTimer * indicatorPulseSpeed) * 0.1f;
                currentJumpIndicator.transform.localScale = Vector3.one * jumpSlamRadius * 2f * scale;
            }
        ...
        while (jumpTimer < jumpDuration)
        {
            ...
            {
                float upT = t * 2f;
                transform.position = Vector3.Lerp(startPos, peakPos, jumpCurve.Evaluate(upT));
            }
            else // Coming down
            {
                float downT = (t - 0.5f) * 2f;
                transform.position = Vector3.Lerp(peakPos, endPos, jumpCurve.Evaluate(downT));
                
                if (currentJumpIndicator)
                {
                    UpdateIndicatorColor(currentJumpIndicator, downT);
                }
            }
    ...
    private void CreateDashIndicator(Vector3 direction)
    {
        if (dashIndicatorPrefab)
        {
            ...
            // Setup line renderer or other visual
    ...
    {
        if (normalCollider) normalCollider.enabled = type == ColliderType.Normal;
        if (dashCollider) dashCollider.enabled = type == ColliderType.Dash;
        if (jumpCollider) jumpCollider.enabled = type == ColliderType.Jump;
    }
    ...
    private void SetState(BossState newState)
    {
        ...
        switch (newState)
        {
            ...
            case BossState.DashOngoing:
                animator.SetTrigger("DashOngoing");
                break;
            ...
            case BossState.JumpOngoing:
                animator.SetTrigger("JumpOngoing");
                break;
